package com.ui; // O tu paquete (ej: com.nombredelgrupo.tp2.ui)

// Importaciones de Java Swing y Gráficos
import javax.imageio.ImageIO;
import javax.swing.JPanel;
import java.awt.Dimension;
import java.awt.Color;
import java.awt.Graphics;
import java.awt.image.BufferedImage;
import java.io.File;
import java.io.IOException;

// Importaciones de tu Modelo (¡Ajusta las rutas de paquete!)
import com.tablero.Tablero;
import com.tablero.Casillero;
import com.tablero.TipoCasillero;

public class PanelJuego extends JPanel {

    public static final int TAMAÑO_TILE = 32;

    // --- Propiedades del Modelo ---
    private Tablero tablero;
    private int nivelZActual; 

    // --- Sprites (Bitmaps) ---
    // Guardamos un sprite para cada tipo de casillero
    private BufferedImage spriteRoca;
    private BufferedImage spriteVacio;  // <-- NUEVO
    private BufferedImage spriteRampa;  // <-- NUEVO
    private BufferedImage spriteAgua;   // <-- NUEVO

    /**
     * Constructor.
     */
    public PanelJuego(Tablero tablero, int zInicial) {
        this.tablero = tablero;
        this.nivelZActual = zInicial;

        int anchoPanel = tablero.getDimensionX() * TAMAÑO_TILE;
        int altoPanel = tablero.getDimensionY() * TAMAÑO_TILE;

        this.setPreferredSize(new Dimension(anchoPanel, altoPanel));
        this.setBackground(Color.BLACK); // Fondo por si un sprite falla
        
        cargarImagenes();
    }

    /**
     * Carga todos los archivos de imagen (bitmaps) desde la carpeta de recursos.
     */
    private void cargarImagenes() {
        try {
            // Usamos los nombres que me diste
            this.spriteRoca = ImageIO.read(new File("src/sprites/roca.png"));
            this.spriteVacio = ImageIO.read(new File("src/sprites/vacio.png")); // <-- NUEVO
            this.spriteRampa = ImageIO.read(new File("src/sprites/rampa.png")); // <-- NUEVO
            this.spriteAgua = ImageIO.read(new File("src/sprites/agua.png"));   // <-- NUEVO

        } catch (IOException e) {
            System.err.println("Error al cargar uno o más sprites de terreno.");
            e.printStackTrace();
        }
    }

    /**
     * Se llama solo cada vez que hay que redibujar.
     */
    @Override
    protected void paintComponent(Graphics g) {
        super.paintComponent(g);

        // Recorremos el tablero (solo en X e Y)
        for (int y = 0; y < tablero.getDimensionY(); y++) {
            for (int x = 0; x < tablero.getDimensionX(); x++) {
                
                Casillero casillero = tablero.getCasillero(x, y, this.nivelZActual);
                
                int pixelX = x * TAMAÑO_TILE;
                int pixelY = y * TAMAÑO_TILE;

                // --- LÓGICA DE DIBUJO ACTUALIZADA ---
                // Ahora decide qué sprite dibujar
                
                if (casillero.getTipo() == TipoCasillero.ROCA) {
                    g.drawImage(this.spriteRoca, pixelX, pixelY, this);
                
                } else if (casillero.getTipo() == TipoCasillero.AGUA) { // <-- NUEVO
                    g.drawImage(this.spriteAgua, pixelX, pixelY, this);
                
                } else if (casillero.getTipo() == TipoCasillero.RAMPA) { // <-- NUEVO
                    g.drawImage(this.spriteRampa, pixelX, pixelY, this);
                
                } else if (casillero.getTipo() == TipoCasillero.VACIO) { // <-- NUEVO
                    g.drawImage(this.spriteVacio, pixelX, pixelY, this);
                
                } else {
                    // Si el tipo es nulo o no se reconoce, se verá el fondo negro
                }
            }
        }
        
        // (Aquí dibujaremos al Personaje y Enemigos)
    }

    /**
     * Cambia el piso que se está viendo.
     */
    public void setNivelZActual(int nuevoNivelZ) {
        this.nivelZActual = nuevoNivelZ;
        
        if (this.nivelZActual >= tablero.getDimensionZ()) {
            this.nivelZActual = tablero.getDimensionZ() - 1;
        }
        if (this.nivelZActual < 0) {
            this.nivelZActual = 0;
        }

        this.repaint(); // Forzamos a que el panel se redibuje AHORA
    } 

}